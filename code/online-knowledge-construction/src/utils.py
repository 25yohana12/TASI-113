...
def get_best_available_gpu():
    ...
    try:
        # Menjalankan perintah nvidia-smi untuk mendapatkan info memori bebas
        command = "nvidia-smi --query-gpu=memory.free --format=csv,nounits,noheader"
        output = subprocess.check_output(command.split()).decode('utf-8')
        
        # Parsing output (list memori bebas dalam MB untuk tiap GPU)
        free_memory = [int(x) for x in output.strip().split('\n')]
        
        # Mencari indeks GPU dengan memori bebas terbanyak
        best_gpu_id = free_memory.index(max(free_memory))
        
        print(f"LOG: GPU Terpilih adalah ID {best_gpu_id} dengan {free_memory[best_gpu_id]}MB bebas.")
        return f"cuda:{best_gpu_id}"
    
    except Exception as e:
        print(f"WARNING: Gagal mendeteksi GPU via nvidia-smi ({e}). Menggunakan cuda:0 default.")
        return "cuda:0"

def get_two_best_gpus_real():
    gpu_info = []

    for i in range(torch.cuda.device_count()):
        free_mem, total_mem = torch.cuda.mem_get_info(i)
        used_mem = total_mem - free_mem
        gpu_info.append((i, free_mem, used_mem))

    # sort by FREE memory (descending)
    gpu_info.sort(key=lambda x: x[1], reverse=True)

    llm_gpu = f"cuda:{gpu_info[0][0]}"
    slm_gpu = f"cuda:{gpu_info[1][0]}"

    print("\n=== GPU SELECTION ===")
    for g in gpu_info:
        print(f"GPU {g[0]} | Free: {g[1]/1024**3:.2f} GB | Used: {g[2]/1024**3:.2f} GB")

    print(f"\nLLM → {llm_gpu}")
    print(f"SLM → {slm_gpu}\n")

    return llm_gpu, slm_gpu