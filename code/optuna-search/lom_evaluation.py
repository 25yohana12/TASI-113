...
class LOMEvalConfig:
    ...
    TEST_DATA_PATH = "dataset/lom_data/test.json"
    OUTPUT_PATH    = "evaluation/lom_model_evaluation.json"
...
class LOMEvaluator:
    """
    Evaluator untuk modul LOM berdasarkan tiga metrik yang ditetapkan
    ...
    def __init__(self, config: LOMEvalConfig):
        ...
        print(f"[*] Merging LOM LoRA from {config.LORA_PATH}...")
        self.model = PeftModel.from_pretrained(base, config.LORA_PATH).merge_and_unload()
        self.model.eval()
    ...
    def run(self, mode: str = "prod", num_samples: int = None):
        ...
        # Laporan akhir
        print("\n" + "=" * 50)
        print("📊 RINGKASAN EVALUASI MODUL LOM")
        print("=" * 50)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluator Modul LOM — EM | ROUGE-L (P/R/F1) | BERTScore (P/R/F1)"
    )
    parser.add_argument(
