...
class LOMEvalConfig:
    ...
    LORA_PATH      = "outputs/lom_model/"
    TEST_DATA_PATH = "dataset/lom_data/test.json"   # diperbaiki: sam_data -> lom_data
    OUTPUT_PATH    = "evaluation/lom_baseline_evaluation.json"
...
# ==========================================
# 3. LOM EVALUATOR (PURE BASE MODEL / BASELINE)
# ==========================================
class LOMEvaluator:
    """
    Evaluator baseline untuk modul LOM (pure base model, tanpa LoRA).
    Menggunakan tiga metrik yang ditetapkan pada subbab 3.5.2:
    Exact Match (EM), ROUGE-L, dan BERTScore.
    Setiap metrik berbasis similarity dilaporkan lengkap: Precision, Recall, F1.
    """

    def __init__(self, config: LOMEvalConfig):
        self.cfg = config
        ...
        print(f"[*] Loading PURE base model from {config.BASE_MODEL}...")
    ...
    def build_prompt(self, item: Dict) -> str:
        sys   = item.get("system", "You are an expert about text-to-SQL and pandas code.")
        instr = item.get("instruction", "")
        if "```SR" not in instr:
            instr += "\n\nGenerate Reasoning (SR):\n```SR\n"
        msgs = [
    ...
    def run(self, mode: str = "prod", num_samples: int = None):
        ...
        for idx, item in enumerate(tqdm(test_data, disable=(mode == "debug"))):
            ...
            if mode == "debug":
                ...
                print(f"ROUGE-L P/R/F1: {rougeL_p:.4f} / {rougeL_r:.4f} / {rougeL_f1:.4f}")
        ...
        # Laporan akhir
        print("\n" + "=" * 50)
        print("📊 RINGKASAN EVALUASI BASELINE LOM")
        print("=" * 50)
        ...
        print(f"  {'-'*48}")
        print(f"  {'BERTScore Precision':<24}: {summary['bert_precision']:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluator Baseline LOM — EM | ROUGE-L (P/R/F1) | BERTScore (P/R/F1)"
    )
    parser.add_argument(
    ...
    evaluator = LOMEvaluator(LOMEvalConfig())
