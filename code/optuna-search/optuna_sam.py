"""
OPTUNA TUNING FOR SAM MODULE
==============================

Schema-Aware Masking task optimization
...
CONFIG_FILE = "train_sam_sampling.yaml"
MODULE_NAME = "SAM"
RESULT_DIR = "tuning_results_sam"
PLOT_DIR = "plots_sam"
N_TRIALS = 30
...
def run_training(trial_num, config):
    ...
    output_dir = f"../../outputs/llamafactory-finetuning/sam_trial_{trial_num}"
    config["output_dir"] = output_dir
    
    # Create trial-specific config file
    trial_config_file = f"train_sam_trial_{trial_num}.yaml"
...
def objective(trial):
    """Optuna objective function - FOCUSED SEARCH"""
    ...
    # 1. LoRA Rank (most impactful)
    lora_rank = trial.suggest_categorical('lora_rank', [8, 16, 32])
    
    # 2. LoRA Alpha (TIED TO RANK)
    lora_alpha = lora_rank * 2  # Fixed 2:1 ratio
    ...
    # Gradient accumulation (maintain effective batch = 16)
